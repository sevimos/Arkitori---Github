package com.mayhem.rs2.content.skill.ranged;
...
public class ToxicBlowpipe {
	...
	public static boolean itemOnItem(Player player, Item itemUsed, Item usedWith) {
		...
		//System.out.println("dart= " + dart.getId());
		//System.out.println("player = " + player.getToxicBlowpipe().getBlowpipeAmmo().getId());
		//System.out.println("player ammo = " + player.getToxicBlowpipe().getBlowpipeAmmo().getAmount());
		if((player.getToxicBlowpipe().getBlowpipeAmmo() != null) && (dart.getId() != player.getToxicBlowpipe().getBlowpipeAmmo().getId())) {
			player.send(new SendMessage("You already have different ammo in your blowpipe."));
			return false;
		}
		else if (usedWith.getId() == 12924 || usedWith.getId() == 12926 || itemUsed.getId() == 12924 || itemUsed.getId() == 12926) {
	...
	public static void degrade(Player player) {
		...
		if (cape != null && (cape.getId() == 10499 || cape.getId() == 10498 || cape.getId() == 13337)) {
