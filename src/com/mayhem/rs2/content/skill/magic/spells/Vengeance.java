package com.mayhem.rs2.content.skill.magic.spells;
...
import com.mayhem.rs2.content.skill.magic.Spell;
...
public class Vengeance extends Spell {
	...
	public boolean execute(Player player) {
		if (player.getMagic().isVengeanceActive()) {
			player.getClient().queueOutgoingPacket(new SendMessage("You have already casted Vengeance!"));
			return false;
		}
		...
		player.getClient().queueOutgoingPacket(new SendString("vengtimer:" + (System.currentTimeMillis() - player.getMagic().getLastVengeance() + 30), -1));
		System.out.println("vengtimer:" + (System.currentTimeMillis() - player.getMagic().getLastVengeance() + 30));
		AchievementHandler.activate(player, AchievementList.CAST_VENGEANCE_350_TIMES, 1);
		return true;
	}
