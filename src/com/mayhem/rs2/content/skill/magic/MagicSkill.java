...
import com.mayhem.rs2.content.skill.magic.spells.HighAlchemy;
import com.mayhem.rs2.content.skill.magic.spells.LowAlchemy;
import com.mayhem.rs2.content.skill.magic.spells.SuperHeat;
...
public class MagicSkill {
	...
	public void activateVengeance() {
		...
		/*player.getClient().queueOutgoingPacket(new SendString("vengtimer:" + (System.currentTimeMillis() - lastVengeance + 30), -1));
		System.out.println("vengtimer:" + (System.currentTimeMillis() - lastVengeance + 30));*/
	}
	...
	private void initTeleport(final int x, final int y, final int z, final TeleportTypes type) {
		...
		HouseManager.exitHouse(player, false);
		teleporting = true;
		player.setTakeDamage(false);
		player.getClient().queueOutgoingPacket(new SendRemoveInterfaces());
		player.getController().onTeleport(player);
	...
	public boolean needsRune(int runeId) {
		...
		switch (runeId) {
		case 556:
			if ((wep == 1381) || (wep == 20730) || (wep == 20733) || (wep == 20736) || (wep == 20739) || (wep == 1397) || (wep == 17293)) {
				return false;
			}
			return true;
		case 554:
			...
			return true;
		case 555:
			if ((wep == 1383) || (wep == 20730) || (wep == 20733) || (wep == 1395) || (wep == 17293) || ((shield != null) && (shield.getId() == 18346))) {
				return false;
			}
			return true;
		case 557:
			if ((wep == 1385) || (wep == 20736) || (wep == 20739) || (wep == 1399) || (wep == 17293)) {
				return false;
			}
			return true;
		}
