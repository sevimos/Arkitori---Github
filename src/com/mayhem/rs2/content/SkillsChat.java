	...
	public static boolean handle(Player player, int buttonId) {
		...
		if (chat.getButton() == 94166) {
			if(!player.getSlayer().hasTask()) {
				player.getUpdateFlags().sendForceMessage("[QC] I currently do not have a slayer task.");
				return false;
			}
			player.getUpdateFlags().sendForceMessage("[QC] My current task is " + player.getSlayer().getAmount() + " " + player.getSlayer().getTask() + ".");
			return false;
		}
