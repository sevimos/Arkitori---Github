package com.mayhem.rs2.content.minigames.weapongame;
...
public class WeaponGame {
	...
	public static void startGame() {
		...
		for (Player players : lobbyPlayers) {
			...
			AchievementHandler.activate(players, AchievementList.PARTICIPATE_IN_WEAPON_GAME, 1);
			AchievementHandler.activate(players, AchievementList.PARTICIPATE_IN_WEAPON_GAME_10_TIMES, 1);
		}
	}
		...
		if (game) {
			if (leader !=  null) {
				if (leader == player) {
					...
					AchievementHandler.activate(leader, AchievementList.WIN_10_WEAPON_GAMES, 1);
