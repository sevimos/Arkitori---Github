...
public class MysteryBoxMinigame {
	 ...
	 */
	private final static int CREDITS_REQUIRED = 10;
	...
	public static void open(Player player) {
		...
		player.send(new SendString("</col>Arkitori Credits: @gre@" + Utility.format(player.getCredits()), 17006));
		player.send(new SendString("Mystery Box is a gambling minigame where you can bet @gre@" + CREDITS_REQUIRED + " </col>Arkitori Credits.", 17008));
	...
	public static boolean can(Player player) {
		...
		if (player.getCredits() < CREDITS_REQUIRED) {
			player.send(new SendMessage(MESSAGE_COLOR + "You do not have enough Arkitori Credits to do this!"));
			return false;
		}
