package com.mayhem.rs2.content.moderation;
...
public class StaffTab {
	...
	public static void handle(Player player, String user, PunishmentType punishment, int... rights) {
		...
		case MUTE:
			...
			target.send(new SendMessage("[ <col=255>Mayhem</col> ] You have been muted for 1 hour!"));
			PlayerSave.save(target);
			player.send(new SendMessage("[ <col=255>Mayhem</col> ] You have successfully muted <col=255>" + name + "</col> for 1 hour!"));			
			break;
			
		case UNMUTE:
			target.setMuted(false);
			target.setMuteLength(0);
			target.send(new SendMessage("[ <col=255>Mayhem</col> ] You have been unmuted!"));
			PlayerSave.save(target);
			player.send(new SendMessage("[ <col=255>Mayhem</col> ] You have successfully unmuted <col=255>" + name + "</col>!"));			
			break;
			
		case BAN:
			target.setBanned(true);
			target.setBanLength(System.currentTimeMillis() + 1 * 3_600_000);
			target.send(new SendMessage("[ <col=255>Mayhem</col> ] You have been banned for 1 hour!"));
			PlayerSave.save(target);
			player.send(new SendMessage("[ <col=255>Mayhem</col> ] You have successfully banned <col=255>" + name + "</col> for 1 hour!"));			
			break;
		...
		case KILL:
			...
			player.send(new SendMessage("[ <col=255>Mayhem</col> ] You have killed <col=255>" + name + "</col>!"));							 									
			break;
