package com.mayhem.rs2.content.dwarfcannon;
...
public class DwarfCannon extends RSObject {
	...
	public DwarfCannon(Player owner, int x, int y, int z) {
		...
		ObjectManager.registerObject(getGameObject());
	...
	public boolean construct(int id) {
		if (stage == 1 && id == 8) {
			...
			stage = ((byte) (stage + 1));
			ObjectManager.removeActive(getGameObject());
			setId(8);
	...
	public Mob[] getMobsInPath() {
		...
		for (Iterator<Mob> mobs = cannonOwner.getClient().getNpcs().iterator(); mobs.hasNext() && attack.isEmpty();) {
			Mob mob = mobs.next();
			int dir = GameConstants.getDirection(Integer.signum(cannonLocation.getX() - mob.getX()),
					Integer.signum(cannonLocation.getY() - mob.getY()));
			if (dir >= 0 && dir <= 7 && DIRECIONS[dir] == this.dir) {
				boolean canAttack = !cannonOwner.getCombat().inCombat() || cannonOwner.inMultiArea()
						|| (cannonOwner.getCombat().inCombat()
								&& cannonOwner.getCombat().getLastAttackedBy().equals(mob));
				boolean clearPath = StraightPathFinder.isProjectilePathClear(getX(), getY(), cannonLocation.getZ(),
						mob.getX(), mob.getY());
	...
	public void rotate(Player player) {
		if (ammunition != 0) {
			player.send(new SendAnimateObject(this != null ? this.fromRSObject() : null, ROTATION_DIRECTIONS[dir]));
		}
	}
