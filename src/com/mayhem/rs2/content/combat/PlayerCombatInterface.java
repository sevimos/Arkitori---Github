...
import com.mayhem.core.task.impl.PlayerDeathTask;
...
import com.mayhem.rs2.content.combat.impl.RingOfRecoil;
...
import com.mayhem.rs2.content.skill.magic.spells.Vengeance;
...
import com.mayhem.rs2.content.skill.melee.BarrowsSpecials;
import com.mayhem.rs2.content.skill.prayer.PrayerBook.Prayer;
import com.mayhem.rs2.content.skill.ranged.BoltSpecials;
...
import com.mayhem.rs2.content.sounds.PlayerSounds;
...
import com.mayhem.rs2.entity.player.net.out.impl.SendEntityFeed;
...
public class PlayerCombatInterface implements CombatInterface {
    ...
    public void hit(Hit hit) {
        ...
        if (hit.getAttacker() != null) {
            if (hit.getAttacker().isNpc()) {
                ...
                if (mob != null && MobConstants.isSkeletalWyvern(mob)) {
                    if (ItemCheck.isWearingElementalShield(player) && (hit.getType() == Hit.HitTypes.MAGIC)) {
                        if (player.hasFireImmunity()) {
                            player.getClient().queueOutgoingPacket(new SendMessage("You resist all of the dragonfire."));
                            hit.setDamage(0);
                        } else {
                ...
                Player attacker = World.getPlayers()[hit.getAttacker().getIndex()];
                if (attacker != null) {
                    if (!player.getController().canAttackPlayer(attacker, player) || !player.getController().canAttackPlayer(player, attacker)) {
                        return;
                    }
                }
        ...
        if (hit.getAttacker() != null) {
            ...
            RingOfSuffering.recoil(player, hit.getAttacker(), hit.getDamage());
    ...
    public boolean isIgnoreHitSuccess() {
        if (player.getCombat().getCombatType() == CombatTypes.RANGED && player.getSpecialAttack().isInitialized()) {
            ...
            if (weapon != null && weapon.getId() == 11235 && (weapon.getId() == 12764) && (weapon.getId() == 12765) && (weapon.getId() == 12766) && (weapon.getId() == 12767) && (weapon.getId() == 12768) && (weapon.getId() == 20408)) {
                return true;
            }
    ...
    public void onAttack(Entity attack, int hit, CombatTypes type, boolean success) {
        ...
        if (!attack.isNpc()) {
            ...
            player.send(new SendEntityFeed(p.getUsername(), p.getLevels()[3] - hit, p.getMaxLevels()[3]));
