...
public class BandosGodswordEffect implements CombatEffect {
	...
	public static final int[] BGS_DRAIN_IDS = { 4, 2, 5, 0, 6, 1 };

	@Override
	public void execute(Player p, Entity e) {
		int id = -1;
		int drain = p.getLastDamageDealt();
		...
		if (e.getLevels()[BGS_DRAIN_IDS[id]] - drain < 0) {
			...
			p.getClient().queueOutgoingPacket(new SendMessage("You drain your opponents " + com.mayhem.rs2.content.skill.Skills.SKILL_NAMES[BGS_DRAIN_IDS[id]] + " down to 0."));
			...
			if (id < BGS_DRAIN_IDS.length - 1) {
				...
				p.getClient().queueOutgoingPacket(new SendMessage("You drain some of your opponents " + com.mayhem.rs2.content.skill.Skills.SKILL_NAMES[BGS_DRAIN_IDS[id]] + "."));
			}
		} else {
