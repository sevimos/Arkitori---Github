		...
		TaskQueue.queue(new WalkToTask(player, g) {
			...
			public void onDestination() {
				...
				if ((player.getInventory().hasSpaceFor(g.getItem())) && (GroundItemHandler.remove(g))) {
					...
					if(g.getItem().getId() == 20526) {
						World.sendGlobalMessage("@red@"+player.getUsername()+" has picked up the event key at wilderness level "+player.getWildernessLevel()+".");
					player.setTeleblockTime(500000000);
					}
				} else {
