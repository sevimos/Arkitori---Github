		...
		case "givepack":
			if (parser.hasNext(2)) {
				try {
					...
					World.sendGlobalMessage("</col>[ @dre@Mayhem </col>] @dre@" + p.determineIcon(p) + " " + Utility.formatPlayerName(p.getUsername()) + "</col> has just reedemed a @dre@" + pack + "</col> credit voucher!");
		...
		case "givemod":
			...
			case "givesupport":
				if (parser.hasNext()) {
					...
					World.sendGlobalMessage("</col>[ @dre@Arkitori </col>] @dre@" + player.determineIcon(player) + " " + Utility.formatPlayerName(player.getUsername()) + " has just been promoted to Player Support! ");
