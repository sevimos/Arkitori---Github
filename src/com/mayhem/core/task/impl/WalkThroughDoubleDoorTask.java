...
public class WalkThroughDoubleDoorTask extends Task {
	...
	public void execute() {
		if (stage == 0) {
			...
			ObjectManager.remove(new GameObject(ObjectManager.BLANK_OBJECT_ID, door.getX1(), door.getY1(), door.getZ(), door.getType(), door.getCurrentFace1()));
			ObjectManager.remove(new GameObject(ObjectManager.BLANK_OBJECT_ID, door.getX2(), door.getY2(), door.getZ(), door.getType(), door.getCurrentFace2()));
			door.append();
