...
public class UltimateHiscores implements Runnable {

	private Player player;
	
	public UltimateHiscores(Player player) {
		this.player = player;
	}
	...
	public void run() {
		try {
			...
			if (!db.init()) {
				System.err.println("[Ultimate]: Failed to update "+name+" highscores. Database could not connect.");
				return;
			}
			
			if (player.getRights() == 2 || player.getRights() == 3 || player.getRights() == 4) {
				System.err.println("Didn't save hiscores for [ULTIMATE IRONMAN] " + player.getUsername());
				return;
			}
			
			PreparedStatement stmt1 = db.prepare("DELETE FROM hs_users_ulti_ironman WHERE username=?");
	...
	public static String generateQuery() {
		...
		sb.append("INSERT INTO hs_users_ulti_ironman (");
		sb.append("username, ");
		sb.append("rights, ");
