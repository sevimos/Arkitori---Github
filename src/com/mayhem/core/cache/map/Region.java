...
public class Region {
	...
	public boolean appendDoor(int oId, int x, int y, int z) {
		...
		MapLoading.addObject(false, door.getCurrentId(), door.getX(), door.getY(), z, door.getType(),
				door.getCurrentFace());
	...
	public static Door getDoor(int x, int y, int z) {
		...
		//ObjectManager.setClipToZero(x, y, z);
	...
	public static boolean objectExists(int objectId, int x, int y, int z) {
		Region r = getRegion(x, y);
		GameObject objManager = ObjectManager.getGameObject(objectId, x, y, z);
		if (objManager != null)
			return true;
		if (r == null) {
			return false;
	...
	public static GameObject getObject(int x, int y, int z) {
		Region region = getRegion(x, y);
		GameObject objManager = ObjectManager.getGameObject(x, y, z);
		if (objManager != null)
			return objManager;
		if (region == null) {
			return null;
		...
		RSObject object = region.objects[z][x - regionAbsX][y - regionAbsY];
		return object == null ? null : object.fromRSObject();
	}
	...
	public int getClip(int x, int y, int z) {
		...
		// System.out.println(z + " " + x + " " + y + " " + (((clips[z][x -
		// regionAbsX][y - regionAbsY] & 0x1280102) != 0) + " || " + (clips[z][x
		// - regionAbsX][y - regionAbsY] == -1)));
		// }
