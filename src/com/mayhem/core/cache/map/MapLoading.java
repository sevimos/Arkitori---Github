...
import com.mayhem.core.cache.ByteStream;
...
public class MapLoading {
	...
	public static void processDoors() {
		...
		for (Iterator<QueuedDoor> k = doorQueue.iterator(); k.hasNext();) {
			...
			GameObject o = MapConstants.getDoubleDoor(i.getId(), i.getX(), i.getY(), i.getZ(), i.getFace());
			...
			if (o != null) {// it's a double door
				...
				ignore.add(new QueuedDoor(-1, o.getLocation().getX(), o.getLocation().getY(), o.getLocation().getZ(),
						-1, -1));
	...
	public static void addObject(boolean beforeLoad, int objectId, int x, int y, int height, int type, int direction) {
		 ...
		 * type, direction, true); } }
		 */
		/*Door door = Region.getDoor(x, y, height);
		if (door != null) {
		...
		if (def.hasActions()) {
			...
			if (beforeLoad) {
				if (def.actions != null && def.name != null) {
					if (objectId == 1586 || objectId == 23156 || objectId == 15516
							|| def.name.toLowerCase().contains("door") && !def.name.toLowerCase().contains("trapdoor")
	...
	public static void removeClippingForVariableObject(int x, int y, int height, int type, int direction,
			boolean flag) {
